<script lang="ts">
	...
	const getMobileChildren = (item: NavigationItem): NavigationItem[] => {
		const children = getChildren(item);
		if (item.title !== "Publications" && item.title !== "Community") {
			return children;
		}
		return children.flatMap((child) => {
			const nestedChildren = (child.children ?? []).filter(showInMenu);
			return nestedChildren.length > 0 ? nestedChildren : [child];
		});
	};
...
{#snippet renderMobileItems(items: NavigationItem[])}
	<ul class="grid grid-cols-12 gap-y-8">
		{#each items.filter(showInMenu) as item (item.id)}
			{@const children = getMobileChildren(item)}
			{@const itemUrl = url(item)}
			<li class="col-span-12">
				{#if children.length > 0}
					<p
						class="text-xs uppercase tracking-widest text-gray-400 mb-5"
					>
						{item.title}
					</p>
				...
				{:else if itemUrl}
					<a
						...
						onclick={closeMobileMenu}
					>
						{#if item.show_icon_mobile !== false}
							<figure
								class="shrink-0 flex items-center justify-center rounded-full h-[48px] w-[48px] mr-3 bg-primary-300/20"
							>
								{@render navIcon(getIcon(item))}
							</figure>
						{/if}
						<p
							class="morph-link-title inline-flex items-center gap-1 text-gray-900 font-medium mb-0.5 lg:mb-1"
...
<header class="relative z-50 h-[50px] lg:h-[70px] md:mt-10">
	...
	<div
		class={[
			"fixed left-0 right-0 top-0 z-10 px-4 pt-7 transition-transform duration-300 lg:hidden",
	...
	>
			...
			<div class="flex min-w-0 items-center">
				<div class="flex h-12 items-center rounded-full px-3 shadow-sm">
					<SiteLogo
						{globals}
						variant="wordmark"
						style="relative z-[2] block w-[9.5rem] shrink-0"
		...
		{#if mobileOpen}
			<div
				class="absolute left-4 right-4 top-full z-40 mt-3 overflow-hidden rounded-xl bg-white shadow-[0_24px_70px_rgba(7,21,42,0.22)]"
	...
	<div class="hidden lg:block h-full items-center">
		{#snippet childNavItem(
			...
			{#if href}
				<a {href} class="flex items-center">
					<figure
						class="shrink-0 flex items-center justify-center rounded-full h-[48px] w-[48px] mr-2 bg-primary-500/15"
					>
						{@render navIcon(icon, true)}
					</figure>
		...
		<nav
			class="mx-auto max-w-6xl h-20 flex items-center justify-between bg-primary-800 rounded-full pr-6 gap-4 pl-6"
		>
			<div class="flex h-full items-center pr-5 pl-1 shrink-0">
				<SiteLogo
					{globals}
					variant="wordmark"
					style="relative z-[2] flex items-center"
					svgClass="relative h-[60px] w-[100%] z-[140]"
					fill="var(--color-white)"
				/>
			</div>
			...
			<NavigationMenu.Root class="relative z-1" viewport={false}>
				<NavigationMenu.List class="gap-0">
					{#each navigation.filter(showInMenu) as item (item.id)}
						...
						<NavigationMenu.Item>
							{#if children.length > 0}
								{@const isMegaMenu = children.some(
									(c) =>
										(c.children ?? []).filter(showInMenu)
											.length > 0,
								)}
								{@const isNarrow =
									!isMegaMenu && children.length <= 3}
								...
								<NavigationMenu.Content
									class="overflow-visible! rounded-xl! shadow-xl! border-0! bg-linear-to-b from-blue-100 to-white mt-4!"
									style="inset-inline-start: 50%; margin-inline-start: {isNarrow
										? '-140px'
										: '-310px'};"
								>
									<!-- arrow -->
									<div
										class="absolute -top-[6px] left-1/2 -translate-x-1/2 w-3 h-3 bg-blue-100 rotate-45 rounded-[2px] shadow-[-1px_-1px_2px_0px_rgba(0,0,0,0.08)] z-10"
									></div>
									{#if isMegaMenu}
										<!-- Mega menu: L2 children are column headers, L3 are the links -->
										<div class="flex w-[600px] gap-2 p-3">
											{#each children as column (column.id)}
												{@const columnChildren = (
													column.children ?? []
												).filter(showInMenu)}
												<div class="flex-1 min-w-0">
													<p
														class="text-[10px] font-semibold uppercase tracking-[0.15em] text-gray-400 px-3 pb-2 pt-1"
													>
														{column.title}
													</p>
													<ul
													...
													>
														{#each columnChildren as child (child.id)}
															{@const ChildIcon =
																getIcon(child)}
															{@const childUrl =
																url(child)}
															<li
																class="icons border border-transparent hover:border-blue-200 hover:bg-blue-50 rounded-md transition-all duration-200 p-3 hover:shadow-lg"
															>
																{@render childNavItem(
																	child,
																	ChildIcon,
									...
									{:else}
										<!-- Standard dropdown: 1 col if ≤3 items, 2 col otherwise -->
										<ul
											class={children.length <= 3
												? "flex flex-col w-[280px] gap-1 p-3"
												: "grid w-[600px] grid-cols-2 gap-1 p-3"}
										>
											{#if item.nav_title}
												<li
													class="col-span-2 px-3 pb-1"
												>
													<p
														class="text-[10px] font-semibold uppercase tracking-[0.15em] text-gray-400"
													>
														{item.nav_title}
													</p>
												</li>
							...
							{:else}
								...
								{#if itemUrl}
									<NavigationMenu.Link
										href={itemUrl}
										class="bg-transparent! text-white text-base font-medium px-5 py-2 hover:bg-white/10! rounded-full transition-colors duration-200 flex-row! {item.show_icon_desktop
											? 'flex items-center gap-2'
											: ''}"
									>
										{#if item.show_icon_desktop}
											<figure
												class="shrink-0 flex items-center justify-center rounded-full h-[32px] w-[32px] bg-white/10"
											>
												{@render navIcon(getIcon(item))}
											</figure>
										{/if}
			...
			<Button
				...
				class="rounded-full bg-white text-primary-900 hover:bg-white/90"
			>
				Contact
			</Button>
		</nav>
